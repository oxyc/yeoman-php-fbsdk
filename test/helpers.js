...
helpers.before = function before(dir) {
  ...
  return function(done) {
    rimraf(dir, function(err) {
      ...
      var lib_files = grunt.file.expandFiles({ dot: true }, path.join(lib, '**'));
      lib_files.forEach(function(file) {
        var src = file.slice(lib.length);
        grunt.file.copy(file, dir + '/lib' + src);
      });
...
helpers.assertFile = function(file, reg) {
  file = path.join(__dirname, 'temp/' + file);
